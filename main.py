...
output_path = '../WsiRegNet/evalution_data/deep_hist_reg' # TO DEFINE
dhr_params = dict()
#dhr_params['segmentation_mode'] = "deep_segmentation"
dhr_params['segmentation_mode'] = "manual"
dhr_params['initial_rotation'] = True
dhr_params['affine_registration'] = True
dhr_params['nonrigid_registration'] = False
initial_rotation_params = dict()
...
affine_registration_params['model_path'] = 'trained_models/affine_model_512' # TO DEFINE
affine_registration_params['affine_type'] = "simple"
dhr_params['affine_registration_params'] = affine_registration_params
...
dhr_params['nonrigid_registration_params'] = None
segmentation_params = dict()
segmentation_params['model_path'] =  None # TO DEFINE
dhr_params['segmentation_params'] = segmentation_params
#dhr_params['segmentation_params'] = None
load_masks = True
###

def run2():

    df = pd.read_csv('../WsiRegNet/landmark_validation.csv')
    
    show = False
    for idx, row in df.iterrows():
        b_loading = time.time()
        current_pair = str(idx)
        if load_masks:
            source, target, source_landmarks, target_landmarks, status, source_mask, target_mask = utils.load_row(row, load_masks=load_masks)
        else:
            source, target, source_landmarks, target_landmarks, status = utils.load_row(row, load_masks=load_masks)
