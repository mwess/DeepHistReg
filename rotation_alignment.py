...
def rotation_alignment(source, target, params, device="cpu"):
    ...
    y_size, x_size = resampled_target.size(0), resampled_target.size(1)
    ...
    initial_ncc = loss_single(resampled_source, resampled_target, device=device, window_size=params['window_size'])
    ...
    centroid_transform = np.array([
        [1, 0, com_x_target - com_x_source],
        [0, 1, com_y_target - com_y_source],
    ])
    centroid_transform = utils.affine2theta(centroid_transform, (y_size, x_size)).view(1, 2, 3).to(device)
    translated_target = utils.tensor_affine_transform(resampled_target.view(1, 1, y_size, x_size), centroid_transform)[0, 0, :, :]

    centroid_ncc = loss_single(translated_target, resampled_source, device=device, window_size=params['window_size'])
    ...
    for i in range(0, 360, angle_step):
        transform = utils.compose_transforms(centroid_transform, utils.affine2theta(utils.generate_rotation_matrix(i, com_x_source, com_y_source), (y_size, x_size)), (y_size, x_size), device=device).view(1, 2, 3).to(device)
        transformed_target = utils.tensor_affine_transform(resampled_target.view(1, 1, y_size, x_size), transform)[0, 0, :, :]
        current_ncc = loss_single(resampled_source, transformed_target, device=device, window_size=params['window_size'])
        if current_ncc < best_ncc:
            ...
            best_angle = i
    if found:
        displacement_field = utils.transform_to_displacement_field(resampled_target.view(1, 1, y_size, x_size), best_transform, device=device)
    else:
        if centroid_ncc < initial_ncc:
            displacement_field = utils.transform_to_displacement_field(resampled_target.view(1, 1, y_size, x_size), centroid_transform, device=device)
        else:
            displacement_field = utils.transform_to_displacement_field(resampled_target.view(1, 1, y_size, x_size), identity_transform, device=device)
    displacement_field = utils.upsample_displacement_field(displacement_field, (2, source.size(0), source.size(1)), device=device)
    # Change line back to original:
    # return displacement_field, 
    return displacement_field, best_ncc, best_angle
...
#
#    centroid_ncc = loss_single(translated_source, resampled_target, device=device, window_size=params['window_size'])
#    #if centroid_ncc < -0.85:
#    #    displacement_field = utils.transform_to_displacement_field(resampled_source.view(1, 1, y_size, x_size), centroid_transform, device=device)
#    #    displacement_field = utils.upsample_displacement_field(displacement_field, (2, source.size(0), source.size(1)), device=device)
#    #    return displacement_field
...
#        current_ncc = loss_single(transformed_source, resampled_target, device=device, window_size=params['window_size'])
#        if current_ncc < best_ncc:
#            found = True
#            best_ncc = current_ncc
#            best_transform = transform
